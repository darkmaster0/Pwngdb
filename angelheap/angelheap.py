...
def getVal(addr):
    val =  int(gdb.execute("x/gx " + addr, to_string=True).split()[1].strip(), 16)
    if DEBUG:
        print("Value at %s : 0x%x"%(addr,val))
    return val
...
def getRegVal(regs):
    ...
    if arch == "x86-64":
        ...
        reg = regs[1]
    val = int(gdb.execute("info register " + reg, to_string=True).split()[1].strip(), 16)
    if DEBUG:
...
class Malloc_bp_ret(gdb.FinishBreakpoint):
    ...
    def stop(self):
        ...
        if arch == "x86-64":
            ...
            chunk["addr"] = value - capsize * 2
        print(">>>>>>>>>We are going to set breakpoint")
        if value == 0:
            return False
...
class Obo_bp_ret(gdb.Breakpoint):
    def __init__(self, arg):
        gdb.Breakpoint.__init__(self,"*"+arg, type=gdb.BP_WATCHPOINT)
        self.arg = arg
        self.oldVal = getVal(arg)


    def stop(self):
        print("Stop at %x %x"%(self.arg,self.oldVal))
        # newVal = getVal(self.arg)
        # codeAddr = getRegVal(["rip","eip"])
        # offset = getoff(codeAddr)
        # if offset < 0: #Mean something wrong
        #     if newVal != self.oldVal:
        #         print(">>>>>Detect Prev_Size overwrote OldVal:0x%x NewVal: 0x%x"%(self.oldVal, newVal))
        self.delete()
...
def checkbelongLibc(addr):
    print("Libc 0x%x"%libcbase())
...
def get_smallbin():
    ...
    for size in range(capsize * 4, max_smallbin_size, capsize * 2):
        ...
        cmd = "x/" + word + hex(
            main_arena + (fastbinsize + 2) * capsize + 8 + idx * capsize * 2)  # calc the smallbin index
        chunkhead["addr"] = int(gdb.execute(cmd, to_string=True).split(":")[1].strip(), 16)
        ...
        try:
            ...
            if status:
                if chunkaddr in fastchunk:
                    ...
                    print('0x{:<18x}0x{:<8x}0x{:<8x}{:<16}{:>18}{:>18}'.format(chunkaddr, prev_size, size, msg, hex(fd),
                                                                               "None"))
                else:
                    msg = "\033[31m Used \033[0m"
                    print('0x{:<18x}0x{:<8x}0x{:<8x}{:<16}{:>18}{:>18}'.format(chunkaddr, prev_size, size, msg, "None",
                                                                               "None"))
            else:
                msg = "\033[1;34m Freed \033[0m"
                print('0x{:<18x}0x{:<8x}0x{:<8x}{:<16}{:>18}{:>18}'.format(chunkaddr, prev_size, size, msg, hex(fd),
